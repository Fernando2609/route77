          ...
          if (intstock == "") {
           ...
           swal
             .fire({
               title: "Inventario",
               text:
                 "¿Realmente desea " + txtMovimiento+" "+ intstock + " productos?",
               icon: "warning",
               showClass: {
...
$("#listMovimiento").on("change", function () {
  movimiento = document.querySelector("#listMovimiento").value;  
});
...
function fntViewMovimiento(COD_PRODUCTO){
...
$("#compromiso_ejecutivo").dataTable({
  ...
  ajax: {
    url: " " + base_url + "/Inventario/getMovimientos/" + COD_PRODUCTO,
    dataSrc: "",
  },
...
$("#modalViewMovimientos").modal("show");

  /* let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
  let ajaxUrl = base_url + "/Inventario/getMovimientos/" + COD_PRODUCTO;
  request.open("GET",ajaxUrl,true);
  request.send();
  $('#modalViewUser').modal('show');
  request.onreadystatechange = function(){
